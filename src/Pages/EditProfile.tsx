...
const EditProfile = () => {
			...
			.then((response) => {
				...
				if (data.success) {
					...
					profileCtx.updateProfile(
						data.link,
						pharmacyNameValidator.inputValue
					);
				}
	...
	const updateProfileHandler: FormEventHandler<HTMLFormElement> = (e) => {
		e.preventDefault();
		if (!pharmacyNameValidator.isInputValid)
			return pharmacyNameValidator.focusHandler();
		if (!emailValidator.isInputValid) return emailValidator.focusHandler();
		if (!districtValidator.isInputValid)
			return districtValidator.focusHandler();
		if (!noValidator.isInputValid) return noValidator.focusHandler();
		if (!addressValidator.isInputValid)
			return addressValidator.focusHandler();
		setUploading(true);
		const url = `${process.env.REACT_APP_API_ENDPOINT}/pharmacist/profile?token=${authCtx.token}`;
